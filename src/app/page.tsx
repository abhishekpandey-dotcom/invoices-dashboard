...
const agingColor = (b: AgingBucket): string =>
  ({ "0-30": "#10b981", "31-60": "#f59e0b", "61-90": "#f97316", "90+": "#ef4444" })[b];
...
function dsoColor(d: number): string {
  if (d > 90) return "#ef4444";
  if (d > 60) return "#f97316";
  if (d > 30) return "#f59e0b";
  return "#10b981";
}
function dsoBg(d: number): string {
...
export default function Dashboard() {
  ...
  const custRows = useMemo(() => {
    ...
    for (const inv of allInvoices) {
      ...
      if (search) {
        ...
        if (![inv.customer_name, inv.customer_email, inv.domain, inv.business, inv.cs_email].some(v => v?.toLowerCase().includes(q))) continue;
      }
  ...
  const agingTot   = useMemo(() => {
    const m = { "0-30": 0, "31-60": 0, "61-90": 0, "90+": 0 } as Record<AgingBucket, number>;
    custRows.forEach(r => { m["0-30"] += r.b0_30; m["31-60"] += r.b31_60; m["61-90"] += r.b61_90; m["90+"] += r.b90plus; });
    return m;
  }, [custRows]);

  // Weighted aggregate DSO for the filtered set
  ...
  // Business-type breakdown — reacts to all filters
  const bizStats = useMemo(() => {
    const map = new Map<string, { total: number; count: number; dsoW: number; b0_30: number; b31_60: number; b61_90: number; b90plus: number }>();
    for (const r of custRows) {
      const biz = r.business || "Other";
      if (!map.has(biz)) map.set(biz, { total: 0, count: 0, dsoW: 0, b0_30: 0, b31_60: 0, b61_90: 0, b90plus: 0 });
      const b = map.get(biz)!;
      b.total += r.total; b.count++;
      if (r.dso_days > 0) b.dsoW += r.dso_days * r.total;
      b.b0_30 += r.b0_30; b.b31_60 += r.b31_60; b.b61_90 += r.b61_90; b.b90plus += r.b90plus;
    }
    return Array.from(map.entries())
      .map(([biz, v]) => ({ biz, total: v.total, count: v.count, dso: v.total > 0 ? Math.round(v.dsoW / v.total) : 0, b0_30: v.b0_30, b31_60: v.b31_60, b61_90: v.b61_90, b90plus: v.b90plus }))
  ...
  function resetFilters() {
    ...
    setB0_30F("All"); setB31_60F("All"); setB61_90F("All"); setB90plusF("All"); setDsoF("All"); setTotalF("All");
  }
    ...
    <div style={S.page}>
      ...
      <div style={S.statsRow}>
        ...
        {BUCKETS.map(b => (
          <div key={b} style={{ ...S.card, borderTop: `3px solid ${agingColor(b)}` }}>
            ...
            <div style={S.cardSub}>
              {custRows.filter(r => b === "0-30" ? r.b0_30 > 0 : b === "31-60" ? r.b31_60 > 0 : b === "61-90" ? r.b61_90 > 0 : r.b90plus > 0).length} customers
            </div>
          </div>
        ))}
      </div>
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(250px,1fr))", gap: 12 }}>
          {bizStats.map(({ biz, total, count, dso, b0_30, b31_60, b61_90, b90plus }) => {
            const [fg, bg, border] = bizColor(biz);
            const pastDue = b31_60 + b61_90 + b90plus;
            return (
              <div key={biz} style={{ background: "#fff", borderRadius: 12, padding: "16px 18px", boxShadow: "0 1px 4px rgba(0,0,0,0.06)", border: `1px solid ${border}`, position: "relative", overflow: "hidden" }}>
                ...
                <div style={{ fontSize: 12, color: "#64748b", marginBottom: 10 }}>{count} customer{count !== 1 ? "s" : ""}</div>
                {/* Aging mini-bar */}
                {total > 0 && (
                  <div style={{ display: "flex", height: 6, borderRadius: 4, overflow: "hidden", gap: 1, marginBottom: 8 }}>
                    {([{ v: b0_30, c: "#10b981" }, { v: b31_60, c: "#f59e0b" }, { v: b61_90, c: "#f97316" }, { v: b90plus, c: "#ef4444" }] as {v:number;c:string}[])
                      .filter(x => x.v > 0)
                ...
                <div style={{ display: "flex", gap: 6, fontSize: 11, flexWrap: "wrap" }}>
                  ...
                  {b90plus > 0 && <span style={{ color: "#ef4444", fontWeight: 600 }}>{fmtUSD(b90plus)}</span>}
                </div>
        ...
        <table style={S.table}>
          <thead>
            <tr>
              ...
              <Th label="90+d"        col="b90plus"  color="#ef4444" />
          ...
          <tbody>
            {custRows.length === 0
              ? <tr><td colSpan={11} style={{ textAlign: "center", padding: 48, color: "#94a3b8" }}>
                  ...
                  return (
                    <>
                      <tr key={r.key}
                        ...
                        <td style={{ ...S.td, color: "#ef4444", fontWeight: 700 }}>{r.b90plus > 0 ? fmtUSD(r.b90plus) : <span style={{ color: "#e2e8f0" }}>—</span>}</td>
                      ...
                      {isExp && invs.map(inv => (
                        <tr key={inv.id} style={{ background: "#f5f3ff", borderLeft: "3px solid #818cf8" }}>
                          ...
                          <td style={{ ...S.td, fontWeight: 700, fontSize: 13, color: "#4338ca" }} colSpan={6}>
          ...
          {custRows.length > 0 && (
            <tfoot>
              <tr style={{ background: "#f1f5f9", fontWeight: 800 }}>
                ...
                <td style={{ ...S.td, color: "#ef4444", fontWeight: 800 }}>{fmtUSD(agingTot["90+"])}</td>
      ...
      <div style={{ textAlign: "center", padding: "16px 0", color: "#94a3b8", fontSize: 12 }}>
        Click ⟳ for live data · Click Total to expand invoices · DSO = Outstanding ÷ (Avg monthly revenue ÷ 30)
      </div>
    </div>
  );
}
...
const S: Record<string, React.CSSProperties> = {
  page:        { maxWidth: 1600, margin: "0 auto", padding: "24px 20px", background: "#f1f5f9", minHeight: "100vh" },
  ...
  statsRow:    { display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(190px,1fr))", gap: 12, marginBottom: 20 },
  ...
  th:          { textAlign: "left", padding: "13px 16px", fontWeight: 700, fontSize: 11, borderBottom: "2px solid #e2e8f0", whiteSpace: "nowrap", background: "#f8fafc", textTransform: "uppercase" as const, letterSpacing: 0.5 },
  td:          { padding: "11px 16px", borderBottom: "1px solid #f1f5f9", verticalAlign: "middle" },
