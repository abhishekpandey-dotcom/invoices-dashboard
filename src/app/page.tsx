...
interface ApiResponse { invoices: InvoiceRowWithUsd[]; inrPerUsd: number; }
interface CustRow {
...
export default function Dashboard() {
  ...
  const custRows = useMemo(()=>{
    const map = new Map<string, CustRow & {sumAmt:number}>();
    for(const inv of allInvoices){
      ...
      if(!map.has(key)) map.set(key,{key,customer_name:inv.customer_name,customer_email:inv.customer_email,domain:inv.domain,customer_status:inv.customer_status,business:inv.business,cs_email:inv.cs_email,account:inv.account,b0_30:0,b31_60:0,b61_90:0,b90plus:0,total:0,invoice_count:0,dso_days:0,sumAmt:0});
      const r=map.get(key)!;
      r.total+=inv.amount_usd; r.invoice_count++; r.sumAmt+=inv.amount_usd;
    ...
    return Array.from(map.values())
      .map(r=>{ const mrr=r.sumAmt/r.invoice_count; return {...r, dso_days: mrr>0 ? Math.round(r.total/(mrr/30)) : 0}; })
      .filter(r=>{
  ...
  },[allInvoices,acctF,bizF,csF,statF,search,b0_30F,b31_60F,b61_90F,b90plusF,dsoF,totalF,sortCol,sortDir]);

  const invoicesByKey = useMemo(()=>{ const map=new Map<string,InvoiceRowWithUsd[]>(); for(const inv of allInvoices){ const key=`${inv.account}::${inv.customer_id}`; if(!map.has(key))map.set(key,[]); map.get(key)!.push(inv); } return map; },[allInvoices]);
  ...
  return(
    <div style={S.page}>
      ...
      <div style={S.tableWrap}>
        <table style={S.table}>
          <thead>
            ...
            <tr style={{background:"#f8fafc"}}>
              <FTh>
                <input style={{...sel,padding:"3px 6px"}} placeholder="Domain..." value={search} onChange={e=>setSearch(e.target.value)}/>
              </FTh>
              <FTh>
                <select style={sel} value={acctF} onChange={e=>setAcctF(e.target.value)}>
                  <option value="All">All</option><option value="India">India</option><option value="US">US</option>
                </select>
              </FTh>
              <FTh>
              ...
              <FTh>
                <select style={sel} value={bizF} onChange={e=>setBizF(e.target.value)}>
                  <option value="All">All</option>{bizTypes.map(t=><option key={t} value={t}>{t}</option>)}
                </select>
              </FTh>
              <FTh>
                <select style={sel} value={csF} onChange={e=>setCsF(e.target.value)}>
                  <option value="All">All</option>{csEmails.map(e=><option key={e} value={e}>{e}</option>)}
                </select>
              </FTh>
              <FTh>
                <select style={sel} value={b0_30F} onChange={e=>setB0_30F(e.target.value)}>
                  <option value="All">All</option><option value="Has balance">Has balance</option>
                </select>
              </FTh>
              <FTh>
                <select style={sel} value={b31_60F} onChange={e=>setB31_60F(e.target.value)}>
                  <option value="All">All</option><option value="Has balance">Has balance</option>
                </select>
              </FTh>
              <FTh>
                <select style={sel} value={b61_90F} onChange={e=>setB61_90F(e.target.value)}>
                  <option value="All">All</option><option value="Has balance">Has balance</option>
                </select>
              </FTh>
              <FTh>
                <select style={sel} value={b90plusF} onChange={e=>setB90plusF(e.target.value)}>
                  <option value="All">All</option><option value="Has balance">Has balance</option>
                </select>
              </FTh>
              <FTh>
          ...
          <tbody>
            {custRows.length===0
              ?<tr><td colSpan={11} style={{textAlign:"center",padding:40,color:"#9ca3af"}}>No customers match filters. <button onClick={resetFilters} style={{color:"#6366f1",background:"none",border:"none",cursor:"pointer",textDecoration:"underline"}}>Clear all filters</button></td></tr>
              :custRows.map((r,i)=>{
                ...
                return(
                  <>{/* eslint-disable-next-line react/jsx-key */}
                    <tr style={{background:i%2===0?"#fff":"#f9fafb"}}>
                      ...
                      <td style={{...S.td,fontWeight:700,color:dsoColor(r.dso_days)}}>{r.dso_days}d</td>
                    </tr>
                    {isExp && invs.map(inv=>(
      ...
      <div style={{textAlign:"center",padding:"16px 0",color:"#9ca3af",fontSize:12}}>
        Auto-refreshes every 5 min · Click ⟳ for live data · Click Total to expand invoices · DSO = Outstanding ÷ (Avg Invoice ÷ 30)
      </div>
    </div>
  );
}
