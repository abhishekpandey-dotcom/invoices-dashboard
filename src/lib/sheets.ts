...
export async function exportToSheets(invoices: InvoiceRow[], dso: CustomerDSO[]): Promise<{ url: string }> {
  ...
  const auth = getAuth();
  const sheets = google.sheets({ version: "v4", auth });
  const invoiceHeaders = ["Account","Invoice #","Customer Name","Customer Email","Stripe Customer ID","Domain","Status","Business","CS Owner","Invoice Status","Amount Due","Currency","Due Date","Days Overdue","Aging Bucket","Description","Invoice URL"];
  const invoiceRows = invoices.map(inv => [inv.account, inv.invoice_number, inv.customer_name, inv.customer_email, inv.customer_id, inv.domain, inv.customer_status, inv.business, inv.cs_email, inv.status, inv.amount_due, inv.currency, inv.due_date ?? "", inv.days_overdue, inv.aging_bucket, inv.description, inv.invoice_url ?? ""]);
  const dsoHeaders = ["Account","Customer Name","Customer Email","Stripe Customer ID","Domain","Status","Business","CS Owner","Currency","Total Outstanding","DSO (Days)","Invoice Count"];
  const dsoRows = dso.map(d => [d.account, d.customer_name, d.customer_email, d.customer_id, d.domain, d.customer_status, d.business, d.cs_email, d.currency, d.total_outstanding, d.dso_days, d.invoice_count]);
  const timestamp = new Date().toISOString().replace("T", " ").slice(0, 19);
  await sheets.spreadsheets.values.batchUpdate({ spreadsheetId: sheetId, requestBody: { valueInputOption: "USER_ENTERED", data: [{ range: "Invoices!A1", values: [[`Last updated: ${timestamp}`, ...Array(invoiceHeaders.length-1).fill("")], invoiceHeaders, ...invoiceRows] }, { range: "DSO!A1", values: [[`Last updated: ${timestamp}`, ...Array(dsoHeaders.length-1).fill("")], dsoHeaders, ...dsoRows] }] } });
