import Stripe from "stripe";
import type { BusinessTypeMap } from "./sheets";

export function getStripeClients() {
  const inKey = process.env.STRIPE_IN_SECRET_KEY;
  const usKey = process.env.STRIPE_US_SECRET_KEY;

  if (!inKey || !usKey) {
    throw new Error(
      "Missing Stripe keys. Set STRIPE_IN_SECRET_KEY and STRIPE_US_SECRET_KEY."
    );
  }

  return {
    india: new Stripe(inKey, { apiVersion: "2024-04-10" }),
    us: new Stripe(usKey, { apiVersion: "2024-04-10" }),
  };
}

export type AgingBucket = "0-30" | "31-60" | "61-90" | "90+";

export interface InvoiceRow {
  id: string;
  account: "India" | "US";
  customer_id: string;
  customer_name: string;
  customer_email: string;
  business_type: string;        // from Google Sheets mapping ("" if not found)
  invoice_number: string;
  status: string;
  amount_due: number;           // in major currency units (e.g. USD, INR)
  currency: string;
  due_date: string | null;      // ISO date string
  days_overdue: number;         // 0 if not yet due
  aging_bucket: AgingBucket;
  invoice_url: string | null;
  description: string;
}

export interface CustomerDSO {
  customer_id: string;
  customer_name: string;
  customer_email: string;
  business_type: string;
  account: "India" | "US";
  total_outstanding: number;
  currency: string;
  dso_days: number;             // weighted average days outstanding
  invoice_count: number;
}

const SKIP_STATUSES = new Set(["paid", "void", "draft", "uncollectible"]);

function agingBucket(daysOverdue: number): AgingBucket {
  if (daysOverdue <= 30) return "0-30";
  if (daysOverdue <= 60) return "31-60";
  if (daysOverdue <= 90) return "61-90";
  return "90+";
}

async function fetchInvoices(
  stripe: Stripe,
  account: "India" | "US"
): Promise<Omit<InvoiceRow, "business_type">[]> {
  const rows: Omit<InvoiceRow, "business_type">[] = [];
  const now = Date.now();

  for (const status of ["open"] as Stripe.Invoice.Status[]) {
    let hasMore = true;
    let startingAfter: string | undefined;

    while (hasMore) {
      const page = await stripe.invoices.list({
        status,
        limit: 100,
        ...(startingAfter ? { starting_after: startingAfter } : {}),
      });

      for (const inv of page.data) {
        if (SKIP_STATUSES.has(inv.status ?? "")) continue;

        const dueDateMs = inv.due_date ? inv.due_date * 1000 : null;
        const daysOverdue =
          dueDateMs && dueDateMs < now
            ? Math.floor((now - dueDateMs) / 86_400_000)
            : 0;

        const customerObj =
          typeof inv.customer === "object" && inv.customer !== null
            ? (inv.customer as Stripe.Customer)
            : null;

        const customerId =
          typeof inv.customer === "string"
            ? inv.customer
            : (inv.customer as Stripe.Customer)?.id ?? "";

        rows.push({
          id: inv.id,
          account,
          customer_id: customerId,
          customer_name:
            customerObj?.name ??
            inv.customer_name ??
            inv.customer_email ??
            "Unknown",
          customer_email: customerObj?.email ?? inv.customer_email ?? "",
          invoice_number: inv.number ?? inv.id,
          status: inv.status ?? "open",
          amount_due: inv.amount_due / 100,
          currency: inv.currency.toUpperCase(),
          due_date: dueDateMs
            ? new Date(dueDateMs).toISOString().split("T")[0]
            : null,
          days_overdue: daysOverdue,
          aging_bucket: agingBucket(daysOverdue),
          invoice_url: inv.hosted_invoice_url ?? null,
          description: inv.description ?? "",
        });
      }

      hasMore = page.has_more;
      if (page.data.length > 0) {
        startingAfter = page.data[page.data.length - 1].id;
      } else {
        hasMore = false;
      }
    }
  }

  return rows;
}

export async function getAllInvoices(
  bizTypeMap: BusinessTypeMap = new Map()
): Promise<{
  invoices: InvoiceRow[];
  dso: CustomerDSO[];
}> {
  const { india, us } = getStripeClients();

  const [indiaRows, usRows] = await Promise.all([
    fetchInvoices(india, "India"),
    fetchInvoices(us, "US"),
  ]);

  // Merge business_type from the map (try exact ID match)
  const invoices: InvoiceRow[] = [...indiaRows, ...usRows].map((inv) => ({
    ...inv,
    business_type: bizTypeMap.get(inv.customer_id) ?? "",
  }));

  // Calculate DSO per customer (group by customer_id + account + currency)
  const customerMap = new Map<string, {
    customer_id: string;
    customer_name: string;
    customer_email: string;
    business_type: string;
    account: "India" | "US";
    currency: string;
    totalAmount: number;
    weightedDays: number;
    count: number;
  }>();

  for (const inv of invoices) {
    const key = `${inv.account}::${inv.customer_id}::${inv.currency}`;
    if (!customerMap.has(key)) {
      customerMap.set(key, {
        customer_id: inv.customer_id,
        customer_name: inv.customer_name,
        customer_email: inv.customer_email,
        business_type: inv.business_type,
        account: inv.account,
        currency: inv.currency,
        totalAmount: 0,
        weightedDays: 0,
        count: 0,
      });
    }
    const c = customerMap.get(key)!;
    c.totalAmount += inv.amount_due;
    c.weightedDays += inv.days_overdue * inv.amount_due;
    c.count++;
  }

  const dso: CustomerDSO[] = Array.from(customerMap.values()).map((c) => ({
    customer_id: c.customer_id,
    customer_name: c.customer_name,
    customer_email: c.customer_email,
    business_type: c.business_type,
    account: c.account,
    currency: c.currency,
    total_outstanding: Math.round(c.totalAmount * 100) / 100,
    dso_days:
      c.totalAmount > 0
        ? Math.round(c.weightedDays / c.totalAmount)
        : 0,
    invoice_count: c.count,
  }));

  dso.sort((a, b) => b.total_outstanding - a.total_outstanding);

  return { invoices, dso };
}
